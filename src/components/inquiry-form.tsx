...
export function InquiryForm() {
  ...
  return <form noValidate onSubmit={submit} className="surface-panel p-6 sm:p-9">
    <div className="grid gap-6">
      <div><Label htmlFor="name">Name</Label><Input className="mt-2 h-14 px-4" id="name" value={values.name} onChange={(e) => field("name", e.target.value)} aria-invalid={!!errors.name} aria-describedby={errors.name ? "name-error" : undefined} />{errors.name && <p id="name-error" className="mt-2 text-sm text-destructive">{errors.name}</p>}</div>
      <div><Label htmlFor="email">Email</Label><Input className="mt-2 h-14 px-4" id="email" type="email" value={values.email} onChange={(e) => field("email", e.target.value)} aria-invalid={!!errors.email} aria-describedby={errors.email ? "email-error" : undefined} />{errors.email && <p id="email-error" className="mt-2 text-sm text-destructive">{errors.email}</p>}</div>
      <div><Label htmlFor="phone">Phone <span className="font-normal text-muted-foreground">(optional)</span></Label><Input className="mt-2 h-14 px-4" id="phone" type="tel" value={values.phone} onChange={(e) => field("phone", e.target.value)} aria-invalid={!!errors.phone} aria-describedby={errors.phone ? "phone-error" : undefined} />{errors.phone && <p id="phone-error" className="mt-2 text-sm text-destructive">{errors.phone}</p>}</div>
      ...
      <button className={cn(buttonVariants({ size: "lg" }), "w-full")} type="submit">Send Inquiry</button>
